...
export default function NewPostPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (summary.trim().length < 10) {
      alert('El contenido del post debe tener al menos 10 caracteres.')
      return
    }
    if (callToActionUrl && !/^https?:\/\/.+/.test(callToActionUrl)) {
      alert('La URL del botón de acción debe empezar por http:// o https://')
      return
    }
    setSubmitting(true)
    try {
      const body: Record<string, unknown> = { topicType, summary: summary.trim(), languageCode: 'es' }
      if (callToActionUrl) {
