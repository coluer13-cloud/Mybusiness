...
export async function syncGbp(userId: string, accessToken: string) {
  ...
  if (services.status === 'fulfilled' && services.value.length) {
    await supabase.from('gbp_services').delete().eq('user_id', userId)
    const rows = services.value.map((s: Record<string, unknown>) => {
      const free = s.freeFormServiceItem as Record<string, unknown> | undefined
      const structured = s.structuredServiceItem as Record<string, unknown> | undefined
      const label = free?.label as Record<string, unknown> | undefined
      return {
        user_id: userId,
        service_id: s.name as string,
        name: (label?.displayName as string) ?? (structured?.serviceTypeId as string) ?? 'Servicio',
        description: (label?.description as string) ?? (structured?.description as string) ?? null,
        category: null,
        price: s.price ?? {},
        is_offered: s.isOffered !== false,
      }
    })
    await supabase.from('gbp_services').insert(rows)
  }
