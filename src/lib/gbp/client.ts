...
export async function createAnswer(
  ...
  const res = await gbpFetch(
    `${GBP_QA_BASE}/${questionName}/answers:upsert`,
    accessToken,
...
export async function deleteAnswer(questionName: string, accessToken: string) {
  await gbpFetch(
    `${GBP_QA_BASE}/${questionName}/answers:delete`,
    accessToken,
    { method: 'DELETE' },
  )
}

export async function listServiceItems(locationName: string, accessToken: string) {
  const res = await gbpFetch(
    `${GBP_INFO_BASE}/${locationName}/serviceItems`,
    accessToken,
  )
  const data = await res.json()
...
export async function deleteServiceItem(serviceItemName: string, accessToken: string) {
  await gbpFetch(`${GBP_INFO_BASE}/${serviceItemName}`, accessToken, { method: 'DELETE' })
}
...
export async function updatePost(
  ...
  const res = await gbpFetch(
    `${GBP_V4_BASE}/${postName}?updateMask=${updateMask}`,
    accessToken,
    { method: 'PATCH', body: JSON.stringify(body) },
  )
  return res.json()
...
export async function createPhoto(
  ...
  const res = await gbpFetch(
    `${GBP_V4_BASE}/${locationName}/media`,
    accessToken,
    { method: 'POST', body: JSON.stringify(body) },
  )
  return res.json()
...
export async function updateOpeningHours(
  ...
  const res = await gbpFetch(
    `${GBP_INFO_BASE}/${locationName}?updateMask=regularHours`,
    accessToken,
    { method: 'PATCH', body: JSON.stringify({ regularHours: { periods } }) },
  )
  return res.json()
}

export async function deleteReviewReply(reviewName: string, accessToken: string) {
  await gbpFetch(`${GBP_V4_BASE}/${reviewName}/reply`, accessToken, { method: 'DELETE' })
}
